@tool class_name BaseRequest extends HTTPRequest
...
func _ready() -> void:
	...
	if (http_proxy.host != null and http_proxy.port != null):
		...
		if !(http_proxy.username.is_empty() and http_proxy.password.is_empty()):
			self._headers["Proxy-Authorization"] = "Basic %s" % UniOperations.basic_auth_str(http_proxy.username, http_proxy.password)
